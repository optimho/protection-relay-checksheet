...
The 670 and 615 forms are split into separate files because their front-panel paths, self-supervision signals, and measurement labels differ significantly (LHMI graphical vs text-based, INTERRSIG vs IED status, IA/IB/IC vs IL1/IL2/IL3).

Sections per check: connection method instructions → self-test status → primary-value measurements → clock check → 12-row event log → ABB 800XA comms check → optional event-buffer clear → sign-off.

Generated by: `generate_SEL_biennial_check.py`, `generate_ABB_670_biennial_check.py`, `generate_ABB_615_biennial_check.py`

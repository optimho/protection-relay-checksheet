...
Lighter-weight check sheets for routine in-service condition monitoring. Three files:

| File | Covers | Tabs |
|------|--------|------|
| `SEL_Biennial_Visual_Check.xlsx` | SEL-300G, SEL-311L, SEL-787, SEL-700G | 1 (generic SEL form) |
| `ABB_670_Biennial_Visual_Check.xlsx` | REG670, RET670 | 1 (670-series form) |
...
Generated by: `generate_SEL_biennial_check.py`, `generate_ABB_670_biennial_check.py`, `generate_ABB_615_biennial_check.py`, `generate_GE889_biennial_check.py`
...
generate_*.py       ← one Python script per relay type (openpyxl/python-docx)
add_branding.py     ← post-processor: applies logo + footer to all xlsx
.gitignore          ← excludes *.pdf (manufacturer manuals — large, local only)
```
...
### ABB Relion 670 family (REG670, RET670)
